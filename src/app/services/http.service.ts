...
postdata(requestdata: any) {
  ...
  getDataForEndpoint(endpoint: any) {
    ...
    // this.isTokenExpired()
    var result = this._http.get(this.serverUrlDemo1 + endpoint, httpOptions).pipe(map(res => res));
  ...
  getData(endpoint:any,requestdata: any) {
    ...
    var result = this._http.post(this.serverUrlDemo + endpoint, JSON.stringify(requestdata),httpOptions).pipe(catchError((error) => {this.openSnackBar(); return throwError(error);}),map(response => response));
    return result;
  }

  getDataForapi1(endpoint:any,requestdata: any) {
    const httpOptions = {
      headers: new HttpHeaders({
        'Content-Type': 'application/json',
        // 'Authorization': this.cookieService.get('jwtToken')
    ...
    var result = this._http.post(this.serverUrlDemo1 + endpoint, JSON.stringify(requestdata),httpOptions).pipe(catchError((error) => {this.openSnackBar(); return throwError(error);}),map(response => response));
    return result;
  }
  ...
  getDataForEndpointbyPost1(endpoint: any) {
  ...
  getDatabyEndpointforblogcount(endpoint: any) {
